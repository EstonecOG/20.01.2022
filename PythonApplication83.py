...
def messagebox(ind:int):
    if askyesno("Вопрос","Закрыть окно?"):
        showinfo("Ответ","Закрываем окно")
    else:
        showinfo("Ответ","Закрываем окно")
        root.destroy()
    newroot=Toplevel() #tk()
    tabs=ttk.Notebook(newroot)
...
Label(text="Ruhmajuhataja tund", bg="purple").grid(row=4, column=10)
...
Exit = Button(messagebox(root),root, text = "ВЫХОД", command = root.quit).grid(row = 6, column = 1)
